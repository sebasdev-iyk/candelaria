...
<body>
    ...
    <header class="bg-candelaria-purple text-white shadow-lg sticky top-0 z-50 h-[80px] flex items-center justify-between px-6 border-b border-purple-800">
        <div class="flex items-center gap-4">
            ...
            <div class="hidden md:flex ml-8 bg-purple-900/50 rounded-full px-4 py-2 border border-purple-700">
                ...
                <input type="text" placeholder="Buscar canal..." class="bg-transparent border-none outline-none text-sm text-white placeholder-gray-400 w-48">
            </div>
        </div>
        ...
        <div class="flex items-center gap-6">
            ...
            <a href="admin.php" target="_blank" class="bg-red-600 hover:bg-red-700 text-white px-5 py-2 rounded-full font-bold text-sm tracking-wide transition-all shadow-lg shadow-red-900/40 flex items-center gap-2">
                ...
                <div class="stream-info">
                    <div class="flex justify-between items-start">
                        <div class="flex gap-4">
                            <div class="relative">
                                <img src="<?= htmlspecialchars($currentStream['thumbnail']) ?>" alt="Avatar" class="streamer-avatar bg-purple-900 object-cover">
                                <?php if ($currentStream['status'] === 'live'): ?>
                                    <span class="absolute bottom-0 right-0 w-3 h-3 bg-red-500 border border-black rounded-full animate-ping"></span>
                                    <span class="absolute bottom-0 right-0 w-3 h-3 bg-red-500 border border-black rounded-full"></span>
                                <?php else: ?>
                                    <span class="absolute bottom-0 right-0 w-3 h-3 bg-gray-500 border border-black rounded-full"></span>
                                <?php endif; ?>
                            </div>
                            <div>
                                ...
                                <div class="flex gap-2 mt-2">
                                    <span class="text-xs bg-purple-900/60 text-purple-200 px-2 py-1 rounded"><?= htmlspecialchars($currentStream['category']) ?></span>
                                    <span class="text-xs bg-purple-900/60 text-purple-200 px-2 py-1 rounded uppercase"><?= htmlspecialchars($currentStream['platform']) ?></span>
                                </div>
                            </div>
                        </div>
                        
                        <div class="flex gap-3">
                            <button onclick="toggleFollow(this)" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg font-semibold transition-colors flex items-center gap-2 group">
                        ...
                        <a href="index.php?id=<?= $rec['id'] ?>" class="block">
                            <div class="channel-card group">
                                <div class="card-thumb">
                                    <img src="<?= htmlspecialchars($rec['thumbnail']) ?>" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                ...
                                <div class="card-info">
                                    <div class="w-8 h-8 rounded-full bg-blue-500 overflow-hidden">
                                        <img src="<?= htmlspecialchars($rec['thumbnail']) ?>" class="w-full h-full object-cover">
                                    </div>
