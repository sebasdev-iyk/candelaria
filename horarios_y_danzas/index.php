    ...
    <script>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                    ...
                    <form id="danzas-search-form" class="mb-6">
                        <div class="flex gap-3">
                            ...
                            <button type="button" id="btn-download-pdf" onclick="downloadDancesPDF()" class="px-4 py-2.5 bg-candelaria-gold text-candelaria-purple rounded-lg text-sm font-bold hover:bg-yellow-400 transition-colors flex items-center gap-2 whitespace-nowrap shadow-sm border border-yellow-400">
        ...
        async function downloadDancesPDF() {
            ...
            try {
                ...
                // Defines the specific order and titles requested by user
                // Mapping strategy: 'dia' column or 'categoria' + logic
                // If 'dia' is missing, fallback to categories.
                ...
                dances.forEach(d => {
                    ...
                    const fecha = (d.fecha || '').toLowerCase(); // Fallback if fecha exists

                    // Logic to assign to buckets
                    // Case 1: Explicit Day in DB (Expected)
                    if (dia.includes('31 ene') || fecha.includes('31 ene') || (cat.includes('origin') && d.orden_concurso >= 1 && d.orden_concurso <= 40 && !dia.includes('feb'))) {
                        ...
                        // Fallback logic if NO day info is present
                        if (cat.includes('origin')) buckets.originarias_d1.data.push(d); // Dump all originary here if date unknown
                ...
                for (const key of sectionOrder) {
                    ...
                    // Sort data
                    // Use orden_concurso for stadium, orden_veneracion for parade
                    ...
                    doc.autoTable({
                        ...
                        columnStyles: {
                            0: { width: 15, halign: 'center', fontStyle: 'bold' }, // N°
                            1: { width: 'auto' }, // Conjunto
                            2: { width: 40 } // Categoria
                        },
                        alternateRowStyles: {
